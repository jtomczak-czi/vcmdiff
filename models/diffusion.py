...
class PoissonDiffusionModel(pl.LightningModule):
    """
    PoissonDiffusionModel Model
    
    Idea:
    Data is transformed from counts to real (by first normalizing x/x.sum() and the applying log).
    Diffusion works on the real space.
    Then, in the last step, real is turned into simplex (by softmax).
    Eventually, rates are presented as a point on the simplex multiplied by the total count.
    Specifically:
    counts -> ratios = counts / total counts -> logits = log(ratios + 1) -> exp(logits) - 1 -> softmax
    """
    ...
    def training_step(self, batch, batch_idx):
        x, _ = batch
        # normalize
        x, total_counts = self.normalize_data(x)
        # TODO: From here, ensure that normalized data are used and the simple loss and the full loss use Poisson
        batch_size = x.shape[0]
        t = torch.randint(0, self.timesteps, (batch_size,), device=self.device)
        loss = self.simple_loss(x, t)
        self.log("train_loss", loss, prog_bar=True, on_step=False, on_epoch=True)
