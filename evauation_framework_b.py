...
def run():
    ...
    csv_logger = CSVLogger("logging_space/vcmdiff_data", name="diffusion")
    ...
    # -- Probabilistic Model --
    model = BinaryDiffusion(nnet=nnet, timesteps=NUM_STEPS, beta_min=1e-4, beta_max=0.02, lr=1e-4, sequence_length=2000,
                            T_max=100)

    # TRAINER
    # -checkpoints
    progress_bar_callback = TQDMProgressBar(refresh_rate=50, leave=True)
    early_stop_callback = EarlyStopping(monitor="val_loss", min_delta=0.00, patience=PATIENCE, verbose=False,
                                        mode="min")
    lr_callback = LearningRateMonitor(logging_interval="step")
    model_checkpoint_callback = ModelCheckpoint(monitor='val_loss', mode='min', save_top_k=5)
    # -trainer
    trainer = pl.Trainer(max_epochs=EPOCHS, accelerator="cpu", devices=1, logger=csv_logger,
                         callbacks=[lr_callback, early_stop_callback, progress_bar_callback, model_checkpoint_callback],
