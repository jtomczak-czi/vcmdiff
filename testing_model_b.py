...
from utils.evaluations import assess_gene_model1d, assess_gene_model1d_b
...
if __name__ == "__main__":
    with torch.no_grad():
        ...
        NUM_STEPS = 50
        # -nnet
        nnet_best = UNetMLPb(in_dim=1000, mid_dim=512, bottleneck_dim=512, num_steps=NUM_STEPS)
        nnet_best.load_state_dict(nnet_new_dict)
        # -diffusion
        model_best = BinaryDiffusion(nnet=nnet_best, timesteps=NUM_STEPS, beta_min=1e-4, beta_max=0.02, lr=1e-4, sequence_length=2000, T_max=100)
        ...
        # RUN ASSESSMENT
        N = 1000
        assess_gene_model1d_b(model_best, result_dir, name, test_dataset, N=N, C=1, D=1000, prior=False)
